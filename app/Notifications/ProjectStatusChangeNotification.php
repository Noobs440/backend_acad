...
class ProjectStatusChangeNotification extends Notification implements ShouldQueue
{
    ...
    public function toArray($notifiable)
    {
        ...
        if ($this->status === 'Approved') {
            $message = 'Bonjour ' . $notifiable->nom_user . ', votre projet "' . $this->project->titre_projet . '" a été approuvé par CollabFacultyRise. Nous vous remercions pour votre contribution.';
        } elseif ($this->status === 'Rejected') {
            $message = 'Bonjour ' . $notifiable->nom_user . ', votre projet "' . $this->project->titre_projet . '" n\'a pas été approuvé par CollabFacultyRise. Nous vous remercions pour votre soumission et vous encourageons à soumettre à nouveau après les modifications nécessaires.';
        }else{
